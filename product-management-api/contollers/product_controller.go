...
import (
	...
	"management-inventaris/rabbitmq"
	"net/http"
	"time"
...
func NewProductController(db *gorm.DB, redisClient *redis.Client, rabbitMQ *rabbitmq.RabbitMQService) *ProductController {
	return &ProductController{db: db, redisClient: redisClient, rabbitMQ: rabbitMQ}
}
...
func (pc *ProductController) CreateProduct(c *gin.Context) {
	...
	// Publish the product data to RabbitMQ
	if err := pc.rabbitMQ.PublishMessage(produk); err != nil {
		c.JSON(500, gin.H{"error": "Failed to publish product to RabbitMQ"})
		return
	}
...
func (pc *ProductController) UpdateProduct(c *gin.Context) {
	...
	// Publish the updated product data to RabbitMQ
	if err := pc.rabbitMQ.PublishMessage(produk); err != nil {
		c.JSON(500, gin.H{"error": "Failed to publish updated product to RabbitMQ"})
		return
	}
...
func (pc *ProductController) DeleteProduct(c *gin.Context) {
	...
	// Publish the deleted product ID to RabbitMQ
	if err := pc.rabbitMQ.PublishMessage(map[string]string{"deleted_product_id": id}); err != nil {
		c.JSON(500, gin.H{"error": "Failed to publish deleted product ID to RabbitMQ"})
		return
	}
...
func (pc *ProductController) UpdateInventory(c *gin.Context) {
	...
	// Publish the updated inventory data to RabbitMQ
	if err := pc.rabbitMQ.PublishMessage(inventaris); err != nil {
		c.JSON(500, gin.H{"error": "Failed to publish updated inventory to RabbitMQ"})
		return
	}
...
func (pc *ProductController) CreateOrder(c *gin.Context) {
	...
	if err := pc.rabbitMQ.PublishMessage(pesanan); err != nil {
		c.JSON(500, gin.H{"error": "Failed to publish order to RabbitMQ"})
		return
	}
